...
class RegisterServiceListener implements ListenerInterface
{
     ...
     * @var ZookeeperAgent
     */
    protected $zookeeperAgent;

    /**
     * @var ConsulAgent | ZookeeperAgent
     */
    ...
    public function __construct(ContainerInterface $container)
    {
        ...
        $this->zookeeperAgent = $container->get(ZookeeperAgent::class);
                    ...
                    foreach ($serviceProtocols as $paths) {
                        foreach ($paths as $path => $service) {
                            ...
                            switch ($service['publishTo']) {
                                case 'consul':
                                    $this->clientAgent = $this->consulAgent;
                                    $this->publishToConsul($address, (int) $port, $service, $serviceName, $path);
                                    break;
                                case 'zookeeper':
                                    $this->clientAgent = $this->zookeeperAgent;
                                    $this->publishToZookeeper($address, (int) $port, $service, $serviceName, $path);
                                    break;
                            }
                        }
                    }
    ...
    protected function publishToConsul(string $address, int $port, array $service, string $serviceName, string $path)
    {
        ...
        $response = $this->clientAgent->registerService($requestBody);
    ...
    protected function publishToZookeeper(string $address, int $port, array $service, string $serviceName, string $path)
    {
        $this->logger->debug(sprintf('Service %s[%s] is registering to the zookeeper.', $serviceName, $path), $this->defaultLoggerContext);
        if ($this->isRegistered($serviceName, $address, $port, $service['protocol'], 'zookeeper')) {
            $this->logger->info(sprintf('Service %s[%s] has been already registered to the zookeeper.', $serviceName, $path), $this->defaultLoggerContext);
            return;
        }
        if (isset($service['id']) && $service['id']) {
            $nextId = $service['id'];
        ...
        if ($response->getStatusCode()) {
            $this->registeredServices[$serviceName][$service['protocol']][$address][$port] = true;
            $this->logger->info(sprintf('Service %s[%s]:%s register to the zookeeper successfully.', $serviceName, $path, $nextId), $this->defaultLoggerContext);
        } else {
            $this->logger->warning(sprintf('Service %s register to the zookeeper failed.', $serviceName), $this->defaultLoggerContext);
        }
    }
    ...
    protected function getLastServiceId(string $name)
    {
        $maxId = -1;
        $lastService = $name;
        $services = $this->clientAgent->services()->json();
    ...
    protected function isRegistered(string $name, string $address, int $port, string $protocol, string $driver = 'consul'): bool
    {
        if (isset($this->registeredServices[$name][$protocol][$address][$port])) {
            return true;
        }
        $response = $this->clientAgent->services();
        if ($response->getStatusCode() !== 200) {
            $this->logger->warning(sprintf('Service %s register to the %s failed.', $name, $driver), $this->defaultLoggerContext);
            return false;
        }

...
use Hyperf\Zookeeper\Health as ZkHealth;
use Hyperf\Zookeeper\HealthInterface as ZkHealthInterface;
...
abstract class AbstractServiceClient
{
    ...
    protected function createNodes(): array
    {
        ...
        if (isset($consumer['registry']['protocol'], $consumer['registry']['address'])) {
            ...
            switch ($registryProtocol = $consumer['registry']['protocol'] ?? '') {
                ...
                case 'zookeeper':
                    $registry = $consumer['registry'] ?? [];
                    $nodes = $this->getNodesFromZookeeper($registry);
                    $refreshCallback = function () use ($registry) {
                        return $this->getNodesFromZookeeper($registry);
                    };
                    break;
                default:
    ...
    protected function getNodesFromZookeeper(array $config): array
    {
        $health = $this->createZookeeperHealth($config);
        $services = $health->service($this->serviceName)->json();
        $nodes = [];
        if(!$services) {
            return $nodes;
    ...
    protected function createZookeeperHealth(array $config): ZkHealthInterface
    {
        if (! $this->container->has(ZkHealth::class)) {
            throw new InvalidArgumentException('Component of \'hyperf/zookeeper\' is required if you want the client fetch the nodes info from zookeeper.');
        }
        return make(ZkHealth::class);
    }
